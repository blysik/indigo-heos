...
class Heos(object):
    ...
    def get_play_state(self):
        ...
        reply = self.send_command(
            'player/get_play_state', {'pid': self._get_player_id()})
        if reply:
    ...
    def get_mute_state(self):
        " get mute state "
        reply = self.send_command(
            'player/get_mute', {'pid': self._get_player_id()})
        if reply:
    ...
    def get_volume(self):
        " get volume "
        reply = self.send_command(
            'player/get_volume', {'pid': self._get_player_id()})
        if reply:
    ...
    def get_queue(self):
        " get queue "
        reply = self.send_command(
            'player/get_queue', {'pid': self._get_player_id()})
        return reply

    def clear_queue(self):
        " clear queue "
        reply = self.send_command(
            'player/clear_queue', {'pid': self._get_player_id()})
        return reply
    ...
    def toggle_mute(self):
        " toggle mute "
        reply = self.send_command(
            'player/toggle_mute', {'pid': self._get_player_id()})
        return reply

    def get_music_sources(self):
        " get music sources "
        reply = self.send_command(
            'browse/get_music_sources', {'range': '0,29'})
        return reply

    def get_browse_source(self, sid):
        " browse source "
        reply = self.send_command(
            'browse/browse', {'sid': sid, 'range': '0,29'})
        return reply

    def play_input_same(self, input):
        " play stream "
        reply = self.send_command(
            'browse/play_input', {'pid': self._get_player_id(), 'input': input})
        return reply

    def play_input_other(self, spid, input):
        " play stream "
        reply = self.send_command(
            'browse/play_input', {'pid': self._get_player_id(), 'spid': spid, 'input': input})
        return reply
